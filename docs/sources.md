...
Records coded `A` in `evidence_level` rest on sources retrieved during
construction in September 2026, listed below. Records coded `B` rest on facts
reported consistently across multiple public sources. Records coded `C` are
analyst judgement from domain knowledge and carry no citation, which is the
...
Company registers and filings (Companies House, SEC EDGAR, national registries)
for ownership and founding dates; ESOMAR and Greenbook directories for firms not
captured here; national market research association member lists, which are the
